...
public class SpannableStringUtils {
     ...
     * @return
     */
    public static void setText(JDialogLsitener.ProtocolListener listener, Context mContext, TextView textView, StringBuilder text, int type, int value, int startPos, int len, String tag) {
        switch (type) {
            ...
            case SPAN_TYPE_STRIKETHROUGH:
                ...
                break;
            case SPAN_TYPE_CLICK:
                textView.setText(getClickSpannable(listener, mContext, text, value, startPos, len, tag));
                break;
            default:
                break;
        }
    ...
    public static SpannableString getClickSpannable(JDialogLsitener.ProtocolListener listener, Context mContext, StringBuilder text, int textColor, int[] startPos, int[] len, String[] tag) {
        SpannableString span = new SpannableString(text);
        if (startPos != null && len != null && startPos.length > 0 && len.length > 0 && startPos.length == len.length) {
            for (int i = 0; i < startPos.length; i++) {
                span.setSpan(new ProtocolClickableSpan(listener, mContext, tag[i], textColor), startPos[i], startPos[i] + len[i], Spanned.SPAN_INCLUSIVE_INCLUSIVE);
            }
        }
    ...
    public static SpannableString getClickSpannable(JDialogLsitener.ProtocolListener listener, Context mContext, StringBuilder text, int textColor, int startPos, int len, String tag) {
        SpannableString span = new SpannableString(text);
        span.setSpan(new ProtocolClickableSpan(listener, mContext, tag, textColor), startPos, startPos + len, Spanned.SPAN_INCLUSIVE_INCLUSIVE);
        return span;
    }
