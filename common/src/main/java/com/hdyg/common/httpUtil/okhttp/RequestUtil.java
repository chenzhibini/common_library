...
class RequestUtil {
    ...
    private int serversLoadTimes = 1, maxLoadTimes = 3;
        ...
        mOkHttpClient = new OkHttpClient()
                ...
                .cookieJar(new CookieJar() {
                    ...
                    public void saveFromResponse(HttpUrl url, List<Cookie> cookies) {
                        //这里把cookies存起来
                        String cookieStr = JsonUtil.parseListToJson(cookies);
                        if (cookieStr.contains("PHPSESSID")){
                            SPUtils.put(SpMsg.COOKIE, cookieStr);
                        }
...
//                            LogUtils.e("cookie==>"+cookieStr);
                            for (int i = 0; i < cookies.size(); i++) {
                                ...
                                if (cookies.contains("PHPSESSID")){
                                    CommonModule.setCookies(cookies.get(i));
                                }
                            }
                        }
                    }
...
//                        List<Cookie> cookies = cookieStore.get(url.host());
                        String jsonStr = SPUtils.get(SpMsg.COOKIE, "");
                        List<Cookie> cookies = null;
                        if (!TextUtils.isEmpty(jsonStr)){
                            cookies = JsonUtil.jsonToArrayList(jsonStr, Cookie.class);
                        }
                ...
                .readTimeout(40, TimeUnit.SECONDS).build();          // 初始化被设置超时时间
    ...
    void execute() {
        mOkHttpClient.newCall(mOkHttpRequest).enqueue(new Callback() {
            ...
            public void onFailure(final Call call, final IOException e) {
                // 如果超时，并未超过指定次数，则重新加载
                if (e.getCause() != null && e.getCause().equals(SocketTimeoutException.class) && serversLoadTimes < maxLoadTimes) {
                    serversLoadTimes++;
                    mOkHttpClient.newCall(mOkHttpRequest).equals(this);
                } else {
                    if (mCallBack != null) {
